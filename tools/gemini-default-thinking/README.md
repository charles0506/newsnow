...
每次開新對話，自動把 [gemini.google.com](https://gemini.google.com) 切換成「**延伸思考**」模式、讓**左側選單預設展開**；也可以順便鎖定慣用模型（例如 `3.1 Pro`）。
...
| 左側選單預設展開 | 每次載入自動把側欄展開；視窗過窄時不動作（免得浮動選單擋內容） |
| 側欄寬度可調並記住 | 直接拖曳側欄右緣，或用選單輸入數值（200–560 px），下次自動套用 |
| 側欄區塊可摺疊 | 「筆記本 / Gem / 近期對話」標題旁出現 ▾，點一下收合，會記住 |
| 自動更新 | 內建 `@updateURL`，之後我修好改版問題你不用重裝 |
| 鎖定預設模型（可選） | 先切模型再開思考；留空就只管思考模式、不動模型 |
| 已開啟就不動作 | 膠囊按鈕若已顯示「延伸 / Thinking」，連選單都不開，畫面不閃 |
| 安全煞車 | 認不出任何勾選狀態時**寧可不點**，避免把已開的思考模式反向關掉 |
| 尊重手動操作 | 你自己關掉思考模式或收合側欄後，腳本不再硬開回去 |
...
| 📂 左側選單預設展開 | 開 | 關掉後完全不碰側欄 |
| 📐 側欄寬度 | 跟隨 Gemini | 輸入 200–560，超出自動夾回範圍；0 或留空＝不改 |
| ↔️ 拖曳調整側欄寬度 | 開 | 側欄右緣會有一條可拖曳的細條 |
| 🗂 側欄區塊摺疊箭頭 | 開 | 關掉就不注入 ▾ |
| ↩️ 全部區塊展開 | — | 一鍵還原所有收合的區塊 |
| 🧪 傾印側欄結構 | — | Gemini 改版時用，把 Console 表格回報給我就能校正選擇器 |
| 🔔 提示訊息 | 開 | 右下角小提示 |
...
側欄部分：讀 `bard-sidenav` 的實際寬度判斷狀態（收合軌道約 72px、展開約 260px 以上），只在「確定是收合」且視窗寬度 ≥ 1000px 時，點 `[data-test-id="side-nav-menu-button"]`（或帶 menu 圖示／選單 aria-label 的按鈕）。點完若寬度沒變大就還原並放棄，找不到明確的開關按鈕則完全不點。
...
用 jsdom 模擬 Gemini 選單與側欄，驗證 15 種情境（含「已開啟時絕不誤關」「改版時安全煞車」「側欄已展開不會被收起」「窄視窗不動側欄」「寬度夾範圍」「只收合指定區塊」）：
...
- 視窗寬度 < 1000px 時不自動展開側欄：該情況下 Gemini 的側欄是覆蓋式浮動選單，展開會直接擋住對話內容。
